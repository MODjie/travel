...
public class PersonalController {
	...
	public ModelAndView toUserInfo(String authorName) {
		...
		ExUser author = null;
		if (authorName != "") {
			author = exUserService.selectByNickName(authorName);
			modelAndView.addObject("author", author);
		}

		String addFocus = getAddFocus(author,currentUser);
	...
	public ModelAndView exprienceList(String currentType, String nickName) {
		...
		if (nickName != null && !currentUser.getNICKNAME().equals(nickName)) {
			author = exUserService.selectByNickName(nickName);
			modelAndView.addObject("author", author);
		} else {
	...
	private String getAddFocus(ExUser author,ExUser currentUser) {
		// 获取当前用户关注的人
		...
		for (FocusDetail focus : myFocusList) {
			if (author != null && focus.getMYFFOCUS().equals(author.getNICKNAME())) {
				addFocus = "no";
				break;
			}
		}
		// 如果作者是当前用户，则addFocus=no
		if (author != null && author.getNICKNAME().equals(currentUser.getNICKNAME())) {
			addFocus = "no";
		}
		return addFocus;
	}
